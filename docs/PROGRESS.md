...
- [x] 笔迹延迟优化: 笔画采样批量保真传输(消除忙时丢点折线)、渲染改事件驱动并去除固定 16ms 延迟、落笔 kick 即时出墨 (2026-08-25, 真机验收通过)
- [x] 渲染流水线与手势体验优化: NEON 4路向量展开加速、Kotlin渲染线程回传零GC分配、双/三指轻点撤销重做即时响应、外接键盘桌面级快捷键直连、WebP图像导出与相册一键保存 (2026-08-31)
- [x] 触控零缓冲与手势跟手优化: 启用 requestUnbufferedDispatch 绕过系统输入队列缓冲、补全双指与单指画布视口变换即时 invalidate 刷新、打通硬件微秒级时间戳、消除手势必须等待笔尖悬停才跳帧刷新的问题 (2026-09-13)
...
- [ ] 全屏沉浸模式
- [ ] 版本控制增强 (缩略图历史)
- [x] 动态非破坏性滤镜图层架构 (基于 Krita KisAdjustmentLayer, 2026-09-14)
