# 液化 Phase 3 调查:rebase / materialize 到底为什么发生

> 目的:在动 [`KisLiquifyTransformWorker`] 之前,把"拖动中反复物化"的**成因**钉死。
> 结论已可从本仓库源码直接判定(rebase 是**我们自己写的**,不在 Krita worker 里);
> 仍需 Krita 源码才能定的只有 §7 的三条(决定 Phase 3 走哪条实现路线)。
> 配套:[液化 V2 改造清单](LIQUIFY-V2-PLAN.md) · [RENDER-OPTIMIZATION.md §4](RENDER-OPTIMIZATION.md)。
...
⇒ Phase 3 的正确方向不是"让 materialize 更快",而是**让重新锚定不需要 materialize**(把旧网格的位移场平移到新网格)。

## 2. 现状调用图(仅列与 rebase 有关的边)
...
且 preview 模式下完全不落盘(`if (!m_liquifyPreview)`)⇒ **拖动中的 `物化 41` 基本全部来自 rebase 那条边**。
...
位移就常达 100~300px ⇒ 每几个输入事件一次 rebase,与真机 `物化 41`(≈1 秒 41 次)吻合。

⇒ 顺带解释了 §4.6.3 那条未验证的方向:单纯放大 `margin` 降低 rebase 频率是**有上限**的 ——
`margin` 逼近 `R` 时,窗口边缘的高斯截断会让笔尖一侧的形变先失真(因为 `3.0·sigma > 1.9·sigma`)。

## 4. 排除法:不是这些原因

| 候选原因 | 是否成立 | 依据 |
|---|---|---|
| Krita 要求周期性重置网格以保数值稳定 | ✗ | worker 内部对每个网格点按 `lambda = exp(-0.5·(d/sigma)²)` 解析计算,无迭代求解、无累积误差项(见 §4.5) |
...
**代价不是"回写 delta"(那本来就小),而是那次 `run()` 必须按整个 `bounds` 跑**(760² 网格 + 全 bounds 补集拷贝),
这就是 `形变 47ms` 的来源。
...
| 路线 | 做法 | 收益 | 前提 / 风险 |
|---|---|---|---|
| A. 加大 `margin`(只调参) | 让内框更贴近 `bounds`,减少 rebase 次数 | rebase 次数 ↓,总物化时间 ↓ | 极其有限(§3),且窗口边缘形变先失真;需真机 A/B |
| B. 延迟 rebase(`deferRebase`) | 交互期只置 `rebasePending = true`,不 flush;抬笔一次 | 拖动中 materialize → **0** | **单独用会静默失效**:笔尖离开窗口后新 dab 形变≈0(§1 表)。只能与 C 配对 |
| C. 逻辑 rebase / 扩展网格(`growGrid`) | 需要重新锚定时,构造**更大**的 worker,把旧网格位移场按双线性采样**写进**新网格,不跑 `run()`、不落盘 | 拖动中 materialize → **0**,且不丢形变 | **API 可行(§7 已确认)**:`transformedPoints()` 是非常量访问器可写;但 worker 内部另有**两个 `KisSpatialContainer` 空间索引**,直接改格点必须同步容器 |
| D. 会话级位移场自持 | 把位移场移出 worker(自己维护),抬笔时才把它交给 Krita 做一次 materialize | 同上 | 等于把真身移出 Krita ⇒ 撤销/选区/Alpha 锁/多层语义要重做,与 §3.2 冲突,**不建议** |
| E. 碰 Krita 内部做 tile 增量 `run()` | 只重算变化格子 | `run()` 成本 ↓ | 需改 Krita 内部 API,风险最高;RENDER-OPTIMIZATION §9 已列为"不建议提前做" |

**建议**:B+C 组合 —— 交互期只做 C(必要时扩网格),**永不 flush**;`bounds` 累积到上限(或抬笔)时做
**唯一一次** materialize。D 与 E 明确不做。

## 7. 三条待确认项 —— 已用 Krita 源码回答

源码:`krita-source/libs/image/kis_liquify_transform_worker.h` 与同名 `.cpp`(两处源码树 `/opt/krita-source` 与
`~/reverie-deps/krita-source` 的该文件 **md5 一致**,见 §10)。

| 问题 | 答案 | 影响 |
|---|---|---|
| 1. `transformedPoints()` 可写吗? | **可写**。头文件里 `QVector<QPointF>& transformedPoints();` 是**非常量**访问器(与 `const QVector<QPointF>& originalPoints() const` 形成对照) | 路线 C 的**前提成立** |
...
| 3. `run()` 对 bounds 补集的处理 | 见 RENDER-OPTIMIZATION §4.2:未遍历瓦片保持 `clear()` 后的透明,所以我们才需要 `seedTransparentFromSource()` 补洞 | **新旧窗口不重叠区域**仍必须走 `run()`/补洞路径 ⇒ 路线 C 只能省掉"重锚定时的 `run()`",不能省掉抬笔那一次 |

**新发现(路线 C 的硬约束)**:worker 内部除两个 `QVector<QPointF>` 之外,还维护**两个空间索引**
...
表现为"笔刷不动了",而不是崩溃)。这是路线 C 实现时必须成对处理的点。

**另外两个可直接利用的 API**:

- `accumulatedStrokesBounds()`:Krita 自己记录的"被笔迹影响过的区域" ⇒ 可作为"何时该 flush"的判据,
  比我们自己累积 `m_liquifyPendingDelta` 更权威;
- `approxChangeRect(rc)` / `approxNeedRect(rc, fullBounds)`:给定 rect 的**结果改变区 / 需要区** ⇒
  是路线 E(按 tile 做增量 `run()`)的现成入口,比"碰 Krita 内部 API"的预期风险低得多,值得重新评估优先级。

## 8. 第一 commit 的埋点方案(**不改任何行为**,只加计数器)
...
| `rebaseCloneMs` | `makeCloneFrom` + `new KisLiquifyTransformWorker` 前后 | 重建成本(§4.6.4 提过设备重建抖动) |
...
落点(与既有 §4.7/§4.16 同构,便于对照):

- C++:[`ReverieCoreMiscTools.cpp`](../app/src/main/cpp/ReverieCoreMiscTools.cpp) 的 rebase 分支 + `liquifyApplyLocked`;
  新增 `ReverieCore::liquifyRebaseStats(qint64*)`(声明进 [`ReverieCore.h`](../app/src/main/cpp/ReverieCore.h))
- JNI:[`reverie_jni_tools.cpp`](../app/src/main/cpp/reverie_jni_tools.cpp) 新增 `liquifyRebaseStats()`
  (**不改** 既有 `liquifyStats` 的返回长度,避免动契约)
- Kotlin:[`ReverieCoreBridge.kt`](../app/src/main/java/com/reverie/paint/core/ReverieCoreBridge.kt) + [`PerfTrace.kt`](../app/src/main/java/com/reverie/paint/core/PerfTrace.kt)
  + debug [`PerfHud.kt`](../app/src/debug/java/com/reverie/paint/perf/PerfHud.kt) 追加一行
- 验证:`scripts/build_native.sh` → 导出符号 / `NEEDED` 闭包比对 → 真机只跑 Test A/B(§9)

## 9. Phase 3 验收标准(先定死,再动代码)

```text
[ ] 拖动阶段 materialize = 0            (HUD: 物化 count 在 drag 中不增长)
[ ] 拖动阶段 rebase 不触发 run()        (HUD: rebaseCount > 0 但 rebaseFlushMs ≈ 0)
[ ] 必要时 rebase 可以 defer            (rebasePending 计数 > 0 且无 materialize)
[ ] 抬笔最多一次最终 materialize
[ ] Final result 与旧路径逐像素一致      (V2 ON vs OFF,pixel diff = 0)
[ ] Reset / Undo / Redo 与旧路径一致
[ ] 关掉 AGSL 预览(路径不受影响)时行为与改动前逐像素一致
[ ] Native ABI / NEEDED 闭包无异常
[ ] Unit tests 全绿
[ ] 真机 4096² + 200px 长笔画 p95/p99 明显改善(不只看均值)
```
...
> 若要改用 `scripts/build_native.sh`,需把 `-PcmakeArgs` 指到上表路径(CMake 默认的 `$HOME/Projects/*`
