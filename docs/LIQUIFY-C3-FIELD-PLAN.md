...
### 8.4 稳定性 + 默认值(高压连续测试之后)

`test_14` 的读数是迄今最好的(`frame 8.0ms p95 16.0ms`), 但**仍是 `预览 引擎`** —— 高压测试跑的还是
经典 CPU 路径, 而那条路在真机上有"大笔刷极端时闪退"的历史(200px 连续拖动时它一秒要在引擎线程上
干 1.07s 的活)。这一版把两件事一起做掉:

1. **场通路改成默认开** —— `debug.reverie.lqfield` 改成三态: 未设 = 平台支持就开, `0` = 强制关,
   `1` = 强制开;设置里的"自动"同理。它把"每 dab 一次 CPU 网格形变 + 每次越界重锚整窗"整段删掉,
   既更丝滑, 也把那条**有崩溃史**的 CPU 路径从高压场景里挪开。挂不上覆盖层 / 超预算 / 非 8bit BGRA
   都会自动回退经典路径(不会"没有预览");要做 A/B 就把"液化位移场"选成 `Krita 网格`。
2. **两端都加硬预算**(专门针对"高压连续测试"的稳定性):
   - Kotlin: 文档像素 > `FIELD_PATH_MAX_PX`(4M)⇒ 不进场通路;低内存设备
     (`ActivityManager.isLowRamDevice`)压到 1M;抬笔回读矩形同样受这个预算约束, 超了就走
     **流式重放补点**(不额外吃大块内存)。
   - C++: `liquifyFieldSource` / `liquifyFieldCommit` 都要求"手势进行中"(`m_liquifyTxnActive`),
     且提交的像素与字节数都过上限闸(`LIQUIFY_FIELD_COMMIT_MAX_PX` = 4M, 与 Kotlin 侧同值)。
   - 另外核对了 `contentBounds()` 的 120ms 兜底: 唯一调用方有 null 判空 ⇒ 不会因为"更早超时"崩溃。

真机再压: 建议 200px / 90% 连续来回拉锯 30s+, 观察 `frame p95`、`场 … dab=N` 是否持续增长
(补点缓冲每段手势清零)、以及抬笔那一次提交的耗时;若仍闪退, 请记下闪退前的最后一次操作(是拖动中
还是抬笔瞬间)与当时的 HUD 读数 —— 那能立刻区分是"场通路"还是"经典路径"的问题。
...
机械验证(本轮收尾): WSL `ninja` 重编原生库 + `llvm-strip` 刷新预编译库(新符号
`liquifyPreviewSourcePixelsInto` 已核对)✅; `compileDebugKotlin` / `testDebugUnitTest` /
`assembleDebug` ✅。真机已由维护者验证场通路效果并确认无闪退。
