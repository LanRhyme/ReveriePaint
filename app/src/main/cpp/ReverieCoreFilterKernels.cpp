...
/* ============================================================
 * ReverieCoreFilterKernels.cpp - 滤镜像素内核实现 (纯 QImage 像素操作,
 * 与 KisPaintDevice/文档状态解耦; 供 FilterPreview 管线与后续注册表滤镜共用。
 * boxBlurH/boxBlurV/filterParallelFor 来自 ReverieCoreInternal.h)
 * ============================================================ */
    ...
    filterParallelFor(0, h, [&](int startY, int endY) {
        for (int y = startY; y < endY; ++y) {
            ...
            for (int x = 0; x < w; ++x) {
                quint8 *px = line + x * 4;
                if (px[3] == 0) continue;
                px[2] = lutR256[px[2]]; // Red
                px[1] = lutG256[px[1]]; // Green
                px[0] = lutB256[px[0]]; // Blue
            }
        }
    });
}
    ...
    filterParallelFor(0, h, [&](int startY, int endY) {
        for (int y = startY; y < endY; ++y) {
            ...
            for (int x = 0; x < w; ++x) {
                ...
                if (px[3] == 0) continue;
                int lum = (px[2] * 299 + px[1] * 587 + px[0] * 114) / 1000;
                quint32 gCol = gradientLut[qBound(0, lum, 255)];
                int gr = (gCol >> 16) & 0xFF;
                int gg = (gCol >> 8) & 0xFF;
                int gb = gCol & 0xFF;
                int ga = (gCol >> 24) & 0xFF;
...
{
    switch (filterType) {
    case 2: return 100;   // 高斯模糊 半径上限
    case 3: return 100;   // 动感模糊 距离上限
    case 4: return 24;    // 锐化 unsharp 邻域
    case 8: return 4;     // Sobel 核
    case 9: return 12;    // 浮雕
    case 11: return 40;   // Glitch 错位上限
    case 18: return 60;   // Bloom 辉光半径上限
    case 19: return 64;   // 投影偏移+模糊余量
    case 21: case 22: case 23: case 26: case 31: case 32: case 33: case 34: return 48; // FX 卷积/位移类保守上限
    default: return 0;    // 逐像素类
    }
}
